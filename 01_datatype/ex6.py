...
#  파이썬 문자열의 중요한 특징
# =========================================================
# 1. 문자열은 (원본불변) 이다 -> (Immutable) (문자열 연산 시 새로운 문자열이 만들어짐)
...
print(a[3:])                        # thon 출력 (끝까지)
print(a[2:4])                       # th 출력 -> 슬라이싱도 할 때마다 새로운 문자열이 생성됨

print(a[::])                        # Python 출력 (step이 양수이면 끝까지, 마지막 인덱스 포함)
...
s = "1301"
print(f"{s[0]}학년 {s[1]}반 {int(s[2]) + int(s[3])}번")                  # ✅ "1학년 3반 1번" 출력

# 2️⃣ 학번을 슬라이싱만을 이용해서 학년, 반, 번호로 잘라내기
print(f"{s[:1]}학년 {s[1:2]}반 {int(s[2:])}번")                  # ✅ "1학년 3반 1번" 출력